...
test("Generating a dynamic segment with unreserved chars does not encode them", function(assert) {
...
test("Generating a dynamic segment with sub-delims or ':' or '@' does not encode them", function(assert) {
...
test("Generating a dynamic segment with general delimiters (except ':' and '@') encodes them", function(assert) {
  // See https://tools.ietf.org/html/rfc3986#section-2.2
  var genDelims = [":", "/", "?", "#", "[", "]", "@"];
  var exclude = [":", "@"];
...
test("Generating a dynamic segment with miscellaneous other values encodes correctly", function(assert) {
  var expectations = [{
    // "/"
    id: "abc/def",
    expected: "abc%2Fdef"
...
test("Throws when generating dynamic routes with an empty string", function(assert) {
  var router = new RouteRecognizer();
...
test("Fails reasonably when bad params passed to dynamic segment", function(assert) {
  var router = new RouteRecognizer();
  router.add([{ "path": "/posts", "handler": "posts" }, { "path": "/*secret/create", "handler": "create" }], { as: "create" });
  router.add([{ "path": "/posts", "handler": "posts" }, { "path": "/:secret/edit", "handler": "edit" }], { as: "edit" });
  ...
  assert.throws(function() {
    router.generate("edit", new String("foo")); //jshint ignore:line
  ...
  assert.throws(function() {
    router.generate("create", new String("foo")); //jshint ignore:line
  }, /You must provide param `secret` to `generate`./, "`new String()` passed.");
...
test("Prevents duplicate additions of the same named route.", function(assert) {
  var router = new RouteRecognizer();
